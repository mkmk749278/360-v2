...
- **FastAPI backend skeleton ✅ shipped (2026-05-04):** `src/api/` package — FastAPI app + Pydantic schemas + engine-snapshot adapters. 9 endpoints (`/api/health`, `/api/pulse`, `/api/signals`, `/api/signals/{id}`, `/api/positions`, `/api/activity`, `/api/auto-mode` GET/POST, `/api/agents`). Read-only except auto-mode POST (delegates to existing `engine.set_auto_execution_mode`). Optional bearer-token auth via `API_AUTH_TOKEN`. Opt-in via `API_ENABLED=true` (default off, won't disturb existing prod). Bound to 127.0.0.1 by default — exposes through reverse proxy. 18 smoke tests in `tests/api/test_api_smoke.py`. Replaces every Lumin app mock with a real source once `API_ENABLED` flips.
- **Phase A4 next:** live with own keys, $50 USDT cap. All safety scaffolding (A1+A2+A3) now in place; B12 doctrine fully satisfied.
- **Next on app track:** flip API_ENABLED behind reverse proxy, then v0.0.5 Lumin update that swaps `mock_data.dart` for an HTTP repository hitting the real endpoints.
