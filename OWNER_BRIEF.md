...
| FUNDING_EXTREME_SIGNAL — 2-fix audit (post-doctrine correction PR #270): (A) telemetry truth — `close <= 0` now emits `invalid_price` instead of conflating with `funding_not_extreme`; (B) TP1 ATR-adaptive cap (1.8R/2.5R/uncapped by atr_percentile).  Originally shipped a third fix — adding FUNDING to the QUIET_SCALP_BLOCK exempt at conf ≥ 60 — which was REVERTED in PR #270 after owner doctrine correction: only paid signals matter, watchlist is scrap.  The exempt at 60 was generating watchlist-tier signals (60-64) that routed to free channel as scrap.  Global QUIET floor (65) now applies uniformly to FUNDING.  No paid signals affected | ✅ 2026-05-04 |
| **QUIET_SCALP_BLOCK doctrine alignment (PR #270)** — Three previously-shipped per-path QUIET exempts removed: QCB fully exempt, DIV_CONT ≥64, FUNDING ≥60.  All three were lowering the QUIET floor below the paid-tier minimum (65), enabling sub-paid signals to route to watchlist tier → free channel.  Per OWNER_BRIEF §2.1a "only the final paid signal matters, watchlist is scrap" — those exempts generated zero business value.  Global `QUIET_SCALP_MIN_CONFIDENCE` (65) now applies uniformly to all 360_SCALP setups in QUIET regime.  No paid signals affected; only watchlist-tier scrap routing eliminated | ✅ 2026-05-04 |
...
- `FUNDING_EXTREME_SIGNAL` — audit #9 (2026-05-02) shipped 3 fixes: telemetry truth + TP1 ATR cap + FUNDING added to QUIET_SCALP_BLOCK exempt at conf ≥ 60.  The exempt portion was REVERTED on 2026-05-04 (PR #270) after owner doctrine correction — it was generating 60-64 watchlist-tier signals that routed to free channel as scrap.  FUNDING signals at 65+ in QUIET continue to reach paid channel via the global floor; sub-65 watchlist signals no longer generated.
