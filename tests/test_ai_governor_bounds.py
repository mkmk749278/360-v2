...
def test_the_call_guard_runs_before_any_per_signal_work(monkeypatch):
...
def test_an_unknown_trigger_timeframe_is_refused_never_defaulted(monkeypatch):
    ...
    res = asyncio.run(gov.sweep({"sig-1": FakeSignal()}, _Store(),
...
def test_maintain_touches_no_position_and_no_exchange(monkeypatch):
    ...
    out = asyncio.run(gov.apply_verdict(_verdict(gov.MAINTAIN, None), sn, m, now=1000.0))
...
def test_panic_refuses_outright_while_its_ceiling_is_unset(monkeypatch):
    ...
    m, sn = _menu_and_snapshot()
    out = asyncio.run(gov.apply_verdict(_verdict(gov.PANIC_CLOSE, None), sn, m, now=1000.0))
    assert out == gov.REFUSE_PANIC_CEILING_UNSET
    assert closed == []
    assert gov.health()["refusals"][gov.REFUSE_PANIC_CEILING_UNSET] == 1


def test_panic_over_its_ceiling_is_refused_not_truncated(monkeypatch):
...
def test_a_level_move_at_1000_members_is_paced_not_burst(monkeypatch):
    ...
    out = asyncio.run(gov.apply_verdict(_verdict(gov.ADJUST_TP, choice), sn, m, now=1000.0))
...
def test_the_sl_arm_never_fights_the_trail_governor(monkeypatch):
    ...
    out = asyncio.run(gov.apply_verdict(_verdict(gov.ADJUST_SL, choice), sn, m, now=1000.0))
    assert out == gov.REFUSE_TRAIL_GOVERNED
    assert moved == []


def test_a_stale_verdict_is_refused_never_applied(monkeypatch):
...
def test_apply_off_is_a_named_refusal_and_the_verdict_is_still_recorded():
...
def test_a_paced_verdict_waits_for_the_next_tick_instead_of_live_locking(monkeypatch):
...
def test_a_cooldown_is_a_throttle_not_a_refusal(monkeypatch):
