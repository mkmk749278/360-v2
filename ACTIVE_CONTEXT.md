...
- **Counter-trend Regime double-penalty fix** (2026-05-03): truth-report data showed LSR / FAR were stuck at Regime score 8 even with near-max SMC scores, while ALSO eating ~8 pts via HTF soft penalty (#268) for being counter-trend. Doctrinally a double-penalty since their non-affinity IS the thesis. New `_REGIME_NEUTRAL_SETUPS` frozenset in `SignalScoringEngine` gives LSR + FAR a 14.0 baseline in non-affinity regimes (still 18 in affinity, soft-penalty for HTF mismatch unchanged). Effect on observed averages: LSR filtered moves from final 60.69 → ~66.69 (clears paid B-tier 65), unblocking the bulk of 3,084 filtered LSR samples per ~24h window. Conservative estimate: 5–10× paid signal volume in mixed-regime markets. 6 new tests confirm scope (only LSR + FAR, not SR_FLIP / QCB / TPE).
