...
- **Soft-penalty per-type instrumentation** (2026-05-03): `confidence_gate` log line now appends `soft_penalties(vwap=X,kz=Y,oi=Z,spoof=W,vol_div=V,cluster=C)` so the truth report can attribute WHICH gate caused the penalty surge. New "Soft-penalty per-type breakdown" section in the truth report. Backward-compatible — old log lines parse cleanly. After redeploy + ~24h, the breakdown will identify the gate to investigate.
