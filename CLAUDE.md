...
- **Reads dominate, and on THIS project the read budget is a HARD CEILING rather than a bill — but not for the reason first written here.** Firestore's no-cost tier is **50,000 document reads/day**, resetting at midnight Pacific.

  The original sentence said the refusal happens to *"a project whose billing account is not in good standing"*. **The owner's console, 2026-09-02, says otherwise**: `lumin-app` is on **Blaze**, billing works (₹0.58 charged for September), and the only line billed is *"Bytes stored over 1 GB, Bandwidth over 10 GB"* — **storage and bandwidth, never reads**. On Blaze, exceeding the no-cost tier normally *charges*; ours was plainly *refused*, and 53,000 reads went by without appearing on the bill.

  So something caps this project at the no-cost tier, and the code cannot see what. The standing hypothesis — **labelled as one, not measured** — is an **App Engine daily spending limit**: this section already records that Datastore-mode usage rolls up under the "App Engine" grouping, and legacy App Engine carries a daily spending limit that produces exactly this `RESOURCE_EXHAUSTED` once free quota is gone. It is one look in the GCP console (App Engine → Settings) and nobody has taken it.

  **Do not "fix" this by raising the cap.** The owner's requirement is *"we don't want to generate any bills in Google cloud"* (2026-09-02), and a cap is what enforces it — it converts a bill into an outage, which is the trade he chose. The engineering answer is to stay far below the ceiling, not to buy headroom with the bill he ruled out. Raising it is an owner decision and an owner-sign-off item.

  The operational consequence is unchanged and is what matters: past the ceiling, **every Firestore-backed path fails together**. On 2026-09-02 that allowance ran out at 00:41 UTC on **53k reads against 25 writes**, and every Firestore-backed path failed together: the keystore (so `list_active_uids` returned empty and every signal fanned out to **zero users**), the kill switch (so `POST /api/kill-switch` 503'd — the emergency stop and the thing it stops fail together), runtime tunables, and the dispatch log. **Count reads per day against 50,000 before merging anything that reads Firestore on a timer or a poll**, and read `read.firestore_reads` on the diag console rather than estimating — it counts *documents* per call site, per process, because a `collection_group` query is one call and N reads.
