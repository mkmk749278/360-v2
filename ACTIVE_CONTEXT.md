...
- **Pre-TP indicator in signal posts** (2026-05-04): Subscribers asked — the dispatched signal post showed Entry / SL / TP1/TP2/TP3 but no mention of pre-TP, leaving the auto-breakeven behaviour invisible at signal time. Added a single line after TP3: `⚡ Pre-TP: +0.20%+ raw (≥+1.3% net @ 10x) → SL → breakeven (auto)`. Conditional: hidden when `PRE_TP_ENABLED=false`, hidden for breakout setups (VSB / BDS / ORB per blacklist). 9 tests in `tests/test_signal_format_pre_tp_indicator.py`.
...
- **Lumin v0.0.2 ✅ shipped:** 5-tab Material 3 NavigationBar (Pulse / Signals / Agents / Trade / Menu), brand shell, agents grid with 14 personas, bottom-sheet detail screens. Installer: `tools/lumin-tabnav.sh`.
- **Lumin v0.0.3 ✅ shipped:** Mock dashboards on Pulse / Signals / Trade — engine snapshot, regime card, daily-loss budget bar, recent-signals list, filter chips (All/Open/Closed) with tap-to-detail bottom sheet, 3-button mode toggle, open positions card, activity feed. Single swap point at `lib/data/mock_data.dart`. Installer: `tools/lumin-v003.sh`.
- **Lumin v0.0.4 ✅ shipped:** Settings drill-downs — every Menu row now opens a real page. Auto-trade (mode + sizing + leverage cap), Pre-TP grab (threshold + ATR floor + regime/setup allowlists), Risk gates (daily-loss kill + leverage + equity floor + streak halt), Agents (14 evaluator toggles + bulk enable/disable), API keys (masked input + testnet toggle + mock test-connection), Subscription (Free vs Pro + Telegram bot CTA — Play Store reader-app compliant), About (full risk disclosure for Play Store). Installer: `tools/lumin-v004.sh`.
- **FastAPI backend skeleton ✅ shipped (2026-05-04):** `src/api/` package — FastAPI app + Pydantic schemas + engine-snapshot adapters. 9 endpoints (`/api/health`, `/api/pulse`, `/api/signals`, `/api/signals/{id}`, `/api/positions`, `/api/activity`, `/api/auto-mode` GET/POST, `/api/agents`). Read-only except auto-mode POST (delegates to existing `engine.set_auto_execution_mode`). Optional bearer-token auth via `API_AUTH_TOKEN`. Opt-in via `API_ENABLED=true` (default off, won't disturb existing prod). Bound to 127.0.0.1 by default — exposes through reverse proxy. 18 smoke tests in `tools/api/test_api_smoke.py`. Replaces every Lumin app mock with a real source once `API_ENABLED` flips. Merged via PR #291.
- **VPS API rollout helper ✅ shipped (2026-05-04):** `tools/setup-vps-api.sh` — idempotent installer that mints an API_AUTH_TOKEN, installs nginx + certbot, writes a hardened reverse-proxy config (rate-limited 60r/min, OPTIONS preflight, vuln-scanner blocks), provisions a Let's Encrypt cert, flips `API_ENABLED=true` in `.env`, restarts the engine container, then smoke-tests `/api/health` + `/api/pulse`. `--rotate-token` for token rotation, `--no-cert` for testing on private nets. Docker-compose now publishes the API on `127.0.0.1:8000` so nginx can proxy without exposing the port to WAN.
- **Lumin v0.0.5 ✅ shipped (2026-05-04):** Backend wiring + repository pattern. New `lib/data/api_client.dart` (HTTP client w/ timeout + retries + bearer auth), `lib/data/repository.dart` (`LuminRepository` interface + `MockRepository` + `HttpRepository`), `lib/data/app_config.dart` (`AppConfigScope` InheritedWidget persisting source/baseUrl/token via `shared_preferences`). API keys settings page extended with backend fields + real "Test connection" button that hits `/api/health` and `/api/pulse`. Pages still consume mocks — v0.0.6 swaps each to FutureBuilder against the live API once the VPS rollout is complete. Installer: `tools/lumin-v005.sh`.
- **Phase A4 next:** live with own keys, $50 USDT cap. All safety scaffolding (A1+A2+A3) now in place; B12 doctrine fully satisfied.
- **Next on app track:** owner runs `setup-vps-api.sh` on the VPS → flips Lumin v0.0.5 to Live mode and verifies via Test connection → v0.0.6 refactors Pulse / Signals / Trade pages to FutureBuilder against the live API.
