...
- `src/unlock_shorts.py` runs as its own task, `_unlock_shorts_loop`, on a
  5-minute cycle. The calendar is read in a fresh interpreter
  (`python -m src.unlock_calendar`), so the 22 MB parse never holds the engine's
  GIL.
- Every insider cliff unlock ≥0.5% of max supply on an admitted perp is
  stamped at the close of T−14 at the executable price.
- Both filters are **stamped, not applied**.
- Rows are walked hourly on 1h klines and closed at T+2 with five variants:
  no stop, 20% stop, 40% stop, BTC-hedged, alt-hedged.
- Refusals are terminal and named: MISSED (never backfilled), LATE (lane start
  vs calendar late), REFUSED (no_price / price_mismatch), CANCELLED,
  INSUFFICIENT.

**Switch and cost.**
- Switch: `unlock_short_lane_enabled` (Measurement), **default ON**, per the
  dark-flag rule and the owner's instruction.
- Public endpoints only. At most 150 calls per cycle, spent before each call.
  Typically one 1h-kline call and one funding call per open row per hour, plus
  one all-symbol ticker (weight 40) on a cycle with an entry or exit.
- New weight entry: `/fapi/v1/fundingRate`. Binance puts it on a separate
  500/5min limit; it is declared 1.

**Where it shows.**
- Ledger `data/unlock_shorts_v1.json`, read by ops `/signals/unlock-shorts`
  (guest-readable, CSV export).
- Probe `unlock_shorts`: calendar stale >30h, or an open row stalled.
- Diag entry `read.unlock_shorts`.
- The ops fixture is generated by `scripts/gen_ops_unlock_shorts_fixture.py`
  from the real lane.

**Watch after deploy.**
- The first cycle is 60s after boot. Read `read.unlock_shorts`: expect
  `calendar_ok` 1.
- On the first read, expect a batch of `LATE (lane_start)` rows: every unlock
  inside the next ~13 days has already passed its entry.
- The first real entries come at the next 00:00 UTC.
- **The first verdict is ~16 days after the first entry.** Nothing is readable
  before then.
...
## SESSION 140 2026-09-02 — a signal-quality audit, and the one discriminator that reproduced
